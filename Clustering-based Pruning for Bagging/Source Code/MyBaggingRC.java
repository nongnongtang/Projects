...
public class MyBaggingRC extends RandomizableParallelIteratedSingleClassifierEnhancer
{
  ...
  /**
    * for the smaller datasets taking away 10% of the training
    * might hurt the accuracy. So it would be interesting to use
    * some artificial random data for the calibration, as an alternative
    * approach. Of course the random data must have the same structure
    * (number of attributes, and attribute types, ...). 
	* I have written a simple randomiser, that takes a dataset 
	* and generates random instances by randomly picking values 
	* from the dataset.
    * @param seed a seed number used for random number generator
	* @param howMany indicating how many instances will be sampled 
	* @param data the data to be sampled from
	* @return the new dataset for clustering
	* @throws Exception if something goes wrong when generating this dataset.
	*/
  
   public static Instances randomData(long seed, int howMany, Instances data) throws Exception {
    Random r = new Random(seed);
    Instances sample = new Instances( data, 0);
    for (int i = 0; i < howMany; i++) {
      double[] x = new double[data.numAttributes()];
      for (int j = 0; j < x.length; j++) {
	x[j] = data.get( r.nextInt(data.size())).value( j);
      }
      sample.add( new DenseInstance(1.0, x));
    }
    return sample;
  }
  ...
  */
  ...
  public void buildClassifier(Instances data) throws Exception {
	...
	Instances random_data = randomData(m_Seed, 1000, m_data);
    Instances probsOfClassifers = validateClassifiers(random_data); 
...
// add a new optioner for clustering classifiers
  ...
  public String toString() {
    StringBuilder sb = new StringBuilder("MyBaggingRC\n\n");
  ...
  */
  ...
  public static void main(String [] argv) {
    runClassifier(new MyBaggingRC(), argv);
  }
}